...
Admin_List = compileFinal "['76561198033399315','_SP_PLAYER_']";		//replace these with your admin player UIDS (steamID64)
	...
	if(!isDedicated) then {
		[] spawn {
			while{true} do {
				...
				((findDisplay 49) displayCtrl 2) ctrlSetText "Anti Cheat By:";
				...
				((findDisplay 49) displayCtrl 523) ctrlSetText "v1.5";
				waitUntil{isNull (findDisplay 49)}
			};
		};
	};	
...
if(!isDedicated) then {
	if(player call AH_AdminCheck) then {
		...
		hint parseText format["Press '%1' to open the admin menu!<br/>Press '%2' to open the spawn menu!<br/>Press F1 F2 and F3 to delete and repair vehicles or open the log menu respectively",(actionKeysNames ["User3",1]),(actionKeysNames ["User4",1])];
	};
};

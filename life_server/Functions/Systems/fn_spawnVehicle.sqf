...
if((_vInfo select 1) == "cop" && (_vInfo select 2) in ["O_MRAP_02_F","C_Offroad_01_F","I_MRAP_03_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","I_Heli_light_03_unarmed_F"]) then
{
	[[_vehicle,"cop_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};

if((_vInfo select 1) == "med" && (_vInfo select 2) in ["C_Offroad_01_F","C_SUV_01_F","B_Heli_Light_01_F","O_Heli_Light_02_unarmed_F"]) then
{
	[[_vehicle,"med_offroad",true],"life_fnc_vehicleAnimate",_unit,false] spawn life_fnc_MP;
};

if((_vInfo select 1) == "adac" && (_vInfo select 2) in ["C_Offroad_01_F","B_Truck_01_transport_F","I_Heli_Transport_02_F","B_Heli_Light_01_F"]) then
{
